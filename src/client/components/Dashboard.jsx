...
const Dashboard = ({ currentUser }) => {
  ...
  const getSeriesForApex = () => {
    ...
    values.slice(1).forEach((row) => {
      const date = ExcelDateToJSDate(row[rh["Date"]]);
      const sleepTime = row[rh["Sleep time (hrs)"]];
      const waterIntake = row[rh["Water (oz)"]];
      const weightVal = row[rh["Weight (lbs)"]];
      const fastVal = row[rh["Fast window (hrs)"]];
      proteinVal += row[rh["Protein (g)"]];
      fatVal += row[rh["Fats (g)"]];
      carbVal += row[rh["Carbs (g)"]];
      // .xaxis.push(date);
      if (date > showSince) {
        count++;
        sleep.push([date.getTime(), sleepTime * 24]);
        water.push([date.getTime(), waterIntake]);
        weight.push([date.getTime(), weightVal]);
        fast.push([date.getTime(), fastVal]);
        macros.push([proteinVal, fatVal, carbVal]);
      }
    });
    setSleepData(...[sleep]);
    setWaterData(...[water]);
